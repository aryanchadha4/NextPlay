...
export async function GET(request: NextRequest) {
  ...
  if (membersRes.error) return serverError(membersRes.error.message);
  if (assigneesRes.error) return serverError(assigneesRes.error.message);

  return NextResponse.json({
    members: membersRes.data,
    taskAssignees: assigneesRes.data,
  });
}
